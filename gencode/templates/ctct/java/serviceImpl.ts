...
export default function(model: BaseModel, moduleName?: string) {
    ...
    const listFieldAttachment = model?.allColumn?.filter(col => col?.controlType === "multiFile");
    ...
    const getsetReference11 = function() {
        ...
        for  (let i = 0; i < refOneList.length; i++) {
            ...
            lines.push(`        if (CommonUtil.isValidId(${col.name})) {`);
    ...
    const importRepository = function() {
        const lines : string[] = [];
        repositoryList.forEach(entityName => {
            lines.push(`import com.viettel.politicalv2.repository.${entityName}Repository;`);
        })
        return lines.join('\n');
    }
    const importService = function() {
        const lines : string[] = [];
        serviceList.forEach(entityName => {
            lines.push(`import com.viettel.politicalv2.service.${entityName}Service;`);
        })
        return lines.join('\n');
    }
    const importEntity = function() {
        const lines : string[] = [];
        repositoryList.forEach(entityName => {
            lines.push(`import com.viettel.politicalv2.repository.model.${entityName};`);
        })
        return lines.join('\n');
    }
    ...
    const getsetReference1n = function() {
        ...
        for  (let i = 0; i < refManyList.length; i++) {
            ...
            if (col.controlType && mapMultiControlType.includes(col.controlType)) {
                ...
                lines.push(`        if (!CommonUtil.isNullOrEmpty(${col.name})) {`)
    ...
    const saveManyList = function() {
        ...
        for  (let i = 0; i < refManyList.length; i++) {
            ...
            if (col.controlType && mapMultiControlType.includes(col.controlType)) {
                ...
                importDto.push(`import com.viettel.politicalv2.repository.dto.${refModel?.entityName}DTO;`);
                ...
                importDto.push(`import com.viettel.politicalv2.service.${refModel?.entityName}Service;`);
        ...
        if (listFieldAttachment instanceof Array && listFieldAttachment.length) {
            listFieldAttachment.forEach(col => {
                importUtils.addImport(`import java.util.List;`);
                importUtils.addImport(`import com.viettel.data.bean.FileInfoBean;`);
                importUtils.addImport(`import com.viettel.data.common.FileStorage;`);
                lines.push(`        if (!CommonUtil.isNullOrEmpty(${model.entityCamelName}DTO.get${UtilsColumn.jsUcfirst(col.name || '')}())) {`);
                lines.push(`            FileStorage.append(FileStorage.FILE_TYPE.${model.entityUCName}_ATTACHMENT, ${model.entityCamelName}DTO.get${UtilsColumn.jsUcfirst(primary?.name || "")}(), ${model.entityCamelName}DTO.get${UtilsColumn.jsUcfirst(col.name || "")}());`);
                lines.push(`        }`);
                getFiles.push(`\n        List<FileInfoBean> ${col.name} = FileStorage.getListFileInfo(FileStorage.FILE_TYPE.${model.entityUCName}_ATTACHMENT, ${primary?.name});`);
                getFiles.push(`            if (!CommonUtil.isNullOrEmpty(${col.name})) {`);
                getFiles.push(`               ${model.entityCamelName}DTO.setFileAttachment("${col.name}", ${col.name});`);
                getFiles.push(`            }`);
            });
        };
    ...
    const exactExportColumn = (column: BaseColumn, index: number) => {
        ...
        str += `String ${column?.name} = ${costName}.findByValue(CommonUtil.NVL(dto.${UtilsColumn.getter(column.name)}()));\n`;
...
package com.viettel.politicalv2.service.impl;
...
${importDto.join('\n')}
import com.viettel.politicalv2.repository.impl.${model.entityName}RepositoryCustom;
${importEntity()}
...
import com.viettel.political.config.TemplateResouces;
import com.viettel.data.common.ExportUtils;
...
${autowiredService()}
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#findAll
     */
    @Override
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#findDetail
     */
    @Override
...
${getFiles.join(`\n`)}
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#findById
     */
    @Override
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#save
     */
    @Override
    @Transactional
...
${saveManyListStr}
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#saveAll
     */
    @Override
    @Transactional
    public List<${model.entityName}DTO> saveAll(List<${model.entityName}DTO> list${model.entityName}DTO) {
        ...
        if (CommonUtil.isNullOrEmpty(list${model.entityName}DTO)) {
            return returnList;
        }
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#delete
     */
    @Override
    @Transactional
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#search${model.entityName}s
     */
    @Override
    ...
    /**
     * @see com.viettel.politicalv2.service.${model.entityName}Service#export${model.entityName}DataTable
     */
    @Override
    public File export${model.entityName}DataTable(HttpServletRequest req, ${model.entityName}Form form) throws Exception {
...
`
UtilsFile.writeFile(`/com.viettel.erp/political-service/src/main/java/com/viettel/politicalv2/service/impl`, `/${model.entityName}ServiceImpl.java`, contentFile);
};
